...
function formatToolParams(params: Record<string, unknown> | undefined): string {
  ...
  const props = params.properties as Record<string, { type: string; description?: string; enum?: string[]; items?: unknown }>;
  const entries = Object.entries(props);
  if (entries.length === 0) return '';

  const required = (params.required as string[]) || [];

  const parts = entries.map(([name, p]) => {
    let s = `  ${name}: ${p.type}`;
    if (p.enum) s += `(${p.enum.join('|')})`;
    if (required.includes(name)) s += ' [必填]';
...
/** 构建工具列表（仅已披露 skill 的 tool 显示完整参数 schema） */
function buildToolSection(disclosedSkillNames: string[]): string {
  const allDefs = toolRegistry.getDefinitions();
  if (allDefs.length === 0) return '';

  const disclosedSet = new Set(disclosedSkillNames);

  // read_skill 始终可见；其余 tool 按 skill 名称匹配（skill 名 → tool 名：- 转 _）
  const visibleDefs = allDefs.filter(d => {
    if (d.function.name === 'read_skill') return true;
    const skillName = d.function.name.replace(/_/g, '-');
    return disclosedSet.has(skillName);
  });

  if (visibleDefs.length === 0) return '';

  let section = '## 工具列表（仅已加载 README 的 skill 所附带的工具显示完整参数）：{\n';
  section += '重要：仅在需要获取你目前无法回答的信息时才调用工具。日常闲聊不需要工具。\n\n';

  for (const def of visibleDefs) {
    const f = def.function;
    const paramsSection = formatToolParams(f.parameters as Record<string, unknown> | undefined);
    section += `**${f.name}** — ${f.description}${paramsSection}\n\n`;
  }
...
function buildToolRules(disclosedSkillNames?: string[]): string {
  const names = disclosedSkillNames || [];
  const skillList = buildSkillList();
  const toolSection = buildToolSection(names);
  return [skillList, toolSection].filter(Boolean).join('\n\n');
}
